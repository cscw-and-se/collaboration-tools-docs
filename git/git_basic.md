...
你可能遇到过这样的麻烦：为了保留不同版本的功能，文件夹里堆满了 `project_v1.zip`, `project_final.zip`, `project_final_but_for_real_this_time.zip`...

这种方式不仅混乱，而且一旦需要找回某个特定版本中的一小段代码，或者想知道某个文件具体在何时被谁修改了，就很难处理。团队合作时问题更大——你无法确认别人手上的版本是不是最新的。
...
2.  **暂存区 (Staging Area / Index)**：这是一个虚拟的“待提交”区域。当你完成了一个小阶段的工作，可以把希望被记录下来的修改添加(add)到暂存区，由你决定本次提交包含哪些改动。
3.  **本地仓库 (Local Repository)**：这是 Git 存放所有项目历史记录的地方（你看到的 `.git` 文件夹就是它）。当你提交(commit)时，Git 会把暂存区里的内容打包，生成一个快照（snapshot），并保存在本地仓库里。
...
![](assets/2025-12-15-22-00-18.png)
...
虽然命令行强大且高效，但在初期，图形化界面能帮你更直观地理解 Git 的工作流程。
  ...
  * **VS Code**
      * VS Code 自带的“源代码管理”面板足以应付基本的 `add` 和 `commit`。但要发挥它的全部潜力，推荐安装以下几个插件：
        1.  **GitLens**: （强烈推荐）它能将 `git blame`（查看每一行代码是谁在何时修改的）信息直接内联显示在你的代码旁，还提供了文件历史和分支对比功能。
