...
选择一套顺手的开发工具，能明显提升日常开发效率。下面按类别整理一些常用工具。

## 1\. 集成开发环境 (IDE)

IDE (Integrated Development Environment) 不只是能写代码的文本编辑器，它集代码高亮、智能补全、调试器、版本控制、终端于一体。选择一款合适的 IDE，是所有开发工作的起点。

### **Visual Studio Code (VS Code)**

  * **一句话介绍**：由微软出品、开源、免费的现代化代码编辑器，也是当今业界的绝对主流。
  * **核心亮点**：
    1.  **极强的扩展性**：VS Code 的核心优势是丰富的插件市场。无论你使用什么语言、什么框架，都可以通过安装插件，把它补全成一个功能接近专用 IDE 的环境。
...
### **JetBrains 全家桶**
  ...
  * **核心亮点**：
    ...
    2.  **出色的代码分析与重构**：这是 JetBrains 的主要优势。它的代码静态分析、智能重构、精准跳转等功能被公认为业界标杆，能帮你写出更健壮、更易维护的代码。
...
* **核心亮点**：
    1.  **无缝跨平台同步**：这是 Termius 的主要优势。你可以在一台电脑上保存的所有服务器连接信息（Hosts）、常用命令片段（Snippets）、连接历史等，都会通过云端安全地同步到你的所有设备上——包括你的 Windows 电脑、MacBook，甚至你的手机和平板。
...
* **简介**：它们是远超系统自带的“启动器”工具。你可以通过一个快捷键呼出输入框，不仅能快速启动应用、计算结果，更强大的是可以通过社区安装各种插件和脚本，实现翻译、查文档、管理剪贴板、窗口布局等功能，适合打造个性化的工作流。
* **官网**：[https://www.raycast.com/](https://www.raycast.com/)

### **Obsidian / Logseq**
* **简介**：基于本地 Markdown 文件的笔记软件。它们通过“双向链接”可以构建一个网状的知识库，适合记录零散的技术笔记、整理课程要点、梳理项目思路。
...
花点时间配置一套适合自己的工具是值得的。工具顺手之后，解决问题的过程也会轻松很多。